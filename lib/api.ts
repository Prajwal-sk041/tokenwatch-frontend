  ...
  async (error: AxiosError) => {
    ...
    const validationMessage =
      Array.isArray(detail) && typeof detail[0]?.msg === "string"
        ...
        : null;
    const safeDetail =
      typeof detail === "string" && detail.length <= 240
        ? detail
        : validationMessage?.slice(0, 240) || null;
    const message =
      error.response?.status === 401
        ? "Your session has expired. Please sign in again."
        : error.code === "ECONNABORTED"
          ? "The request timed out. Please try again."
          : safeDetail || "Unable to complete the request. Please try again.";
    return Promise.reject(new SafeApiError(message));
  },
);

export const register = (email: string, password: string, full_name = "") =>
  api.post("/auth/register", { email, password, full_name });
export const login = (email: string, password: string) =>
  api.post("/auth/login", { email, password });
export const logout = () => api.post("/auth/logout");
export const getMe = () => api.get("/auth/me");
export const updateMe = (full_name: string) =>
...
export const sendTestEvent = (
  org: string,
  data: { sdk_key: string; provider: string; model: string },
) => api.post(`/onboarding/${org}/test-event`, data);
export const createSdkKey = (org: string, data: Record<string, unknown>) =>
  api.post(`/sdk-keys/${org}`, data);
export const listSdkKeys = (org: string) => api.get(`/sdk-keys/${org}`);
export const rotateSdkKey = (org: string, id: string) =>
  api.post(`/sdk-keys/${org}/${id}/rotate`);
export const revokeSdkKey = (org: string, id: string) =>
...
export const createBudget = (org: string, data: Record<string, unknown>) =>
  api.post(`/budgets/${org}`, data);
export const updateBudget = (
  org: string,
  id: string,
  data: Record<string, unknown>,
) => api.patch(`/budgets/${org}/${id}`, data);
export const deleteBudget = (org: string, id: string) =>
  api.delete(`/budgets/${org}/${id}`);
export const getMembers = (org: string) =>
  api.get(`/organizations/${org}/members`);
export const inviteMember = (
  org: string,
  data: { email: string; role: string },
) => api.post(`/organizations/${org}/invites`, data);
export const updateMember = (org: string, id: string, role: string) =>
  api.patch(`/organizations/${org}/members/${id}`, { role });
export const removeMember = (org: string, id: string) =>
  api.delete(`/organizations/${org}/members/${id}`);
export const cancelInvite = (org: string, id: string) =>
  api.delete(`/organizations/${org}/invites/${id}`);
export const getPlans = () => api.get("/subscriptions/plans");
export const getSubscription = (org: string) =>
...
export const createCheckout = (
  org: string,
  plan_code: string,
  billing_interval: "month" | "year" = "month",
  coupon_code?: string,
) =>
  api.post(`/billing/${org}/checkout`, {
    plan_code,
    billing_interval,
    coupon_code: coupon_code || undefined,
  });
export const createBillingPortal = (org: string) =>
  api.post(`/billing/${org}/portal`, { return_path: "/dashboard/billing" });
export const getAdminOverview = () => api.get("/admin/overview");
export const getAdminResource = (resource: string, page = 1) =>
  api.get(`/admin/${resource}`, { params: { page } });
