...
export default function AccountSettingsPage() {
  ...
  >([]);
  const loadSessions = () =>
    getSessions().then((response) => setSessions(response.data));
  useEffect(() => {
    getMe().then((response) => setProfile(response.data));
    void loadSessions();
  }, []);
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    setProfileMessage("");
    try {
      await updateMe(profile.full_name);
  ...
  async function downloadExport() {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `tokenwatch-export-${new Date().toISOString().slice(0, 10)}.json`;
      anchor.click();
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Download a portable JSON copy of your profile, organizations,
          budgets, alerts, and newest usage events. Secrets and provider keys
          are never included.
        </p>
        <button
      ...
      <Card>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              For account safety, a new sign-in replaces the previous device
              session. Team members must use separate invited accounts so
        ...
        <div className="mt-4 space-y-3">
              ...
              >
                ...
                <button
                  onClick={async () => {
                    ...
                    router.replace("/login?session_revoked=1");
                  }}
          ...
          {!sessions.some((session) => !session.revoked_at) && (
